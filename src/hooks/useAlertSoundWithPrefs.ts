...
export function useAlertSoundWithPrefs() {
  ...
  const playAlertSound = useCallback((priority: 'low' | 'medium' | 'high' | 'critical') => {
    ...
    if (!preferences?.sound_enabled) {
      console.log('Sound alerts disabled in preferences');
      return;
    }

    // Check priority preferences
    if (priority === 'critical' && !preferences?.critical_alerts) {
      console.log('Critical alerts disabled in preferences');
      return;
    }

    if (priority === 'high' && !preferences?.high_priority_alerts) {
      console.log('High priority alerts disabled in preferences');
      return;
    }
    ...
    const volume = preferences?.sound_volume ?? 0.5;
